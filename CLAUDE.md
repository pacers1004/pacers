...
## § 28: Feature 05 — Daily Pacer 첫인상 투표 (2026-06-19 기획)

> **상태**: UI 구현 완료 (Bubble 버블 에디터에 화면 존재) / DB 설계 및 워크플로우 미구현

### 개요
- **기능**: Daily Pacer 상세 화면에서 해당 페이서의 첫인상을 3가지 중 투표
- **위치**: Daily Pacer 상세 화면 (`rg_recomend_DailyPacer` 카드 탭 → 상세 팝업/페이지)
- **목적**: 매칭 신청("같이 달려요") 전 단계 설득력 강화 → 클릭율 ↑ → 티켓 소모 ↑ → 수익 직결

### 현재 UI (Bubble에 구현됨 ✅)

**화면 구성:**
- 상단: 페이서 프로필 사진 + "페이서 [닉네임] / 1km [페이스] 페이스 ✨"
- 투표 섹션: "Q.[닉네임]의 첫인상은 어떤가요?" 질문 텍스트
- 투표 버튼 3개:
  - ⚡ **페이스 딱 맞겠다** — 페이스 적합도 표현
  - 🥇 **진짜 러너다** — 진지함/실력 표현
  - 🔥 **꾸준함이 보여** — 꾸준함/지속성 표현
- 하단: 프로필 섹션 (나이/성별/키/지역/나와의 거리)
- 고정 하단: "같이 달려요" 버튼 (티켓 소모)

### 미구현 항목 (다음 작업)

**Bubble DB 설계 (아직 안 됨 ❌)**
- 투표 결과를 어떻게 저장할지 미결정
- 설계 옵션 A: `User` 테이블에 필드 추가 (pacer_vote_pace, pacer_vote_runner, pacer_vote_steady: number)
- 설계 옵션 B: 별도 `PacerVote` 데이터 타입 생성 (voter, target_pacer, vote_type, created_date)
- 추천: **옵션 B** (중복 투표 방지, 투표 이력 추적 가능)

**Bubble 워크플로우 (아직 안 됨 ❌)**
- 투표 버튼 클릭 시 → DB 저장 워크플로우
- 중복 투표 방지 로직 (같은 페이서에 재투표 불가 or 변경 가능 여부 결정 필요)
- 투표 결과 집계 표시 (투표 후 % 결과 보여주기 or 숨기기 결정 필요)
- 투표 완료 → "같이 달려요" 버튼 활성화 유도 (UX 흐름)

### 기획 미결정 사항 (대표님 결정 필요)
1. **중복 투표**: 같은 페이서에 하루 1회만? or 변경 가능?
2. **결과 공개**: 투표 후 집계 % 보여주기? or 투표만 기록하고 숨기기?
3. **투표와 매칭 연동**: 투표하면 티켓 차감 없이 매칭 시도율 ↑? or 독립 기능?
4. **투표 대상**: 오늘 카드로 뜬 페이서만? or 모든 페이서 상세에서?

### 리텐션/수익 임팩트 분석
- 투표 = 저마찰 인터랙션 (티켓 불필요) → 상세 화면 체류시간 ↑
- 체류시간 ↑ → "같이 달려요" 클릭율 ↑ (인지적 관성)
- 매칭 신청 ↑ → 티켓 소모 ↑ → 인앱결제 ↑
- 투표 데이터 → 미래 매칭 스코어 보조 지표로 활용 가능
