...
- **현재**: 국내 버블앱 순위 19위, 유저 1,125명 라이브 서비스 (2026-06-29 확인)
...
### ✅ Auto Mode 작업 기준 (2026-06-20 확정)

| 구분 | 작업 예시 | 처리 방식 |
|------|----------|----------|
| **일반 작업** | 파일 수정, git push, Vercel 배포, JSON 업데이트, MD 문서 수정 | 바로 진행 |
| **위험한 작업** | DB 삭제, 라이브 코드 대규모 변경, 되돌리기 어려운 작업 | **반드시 확인 후 진행** |
| **방향 결정** | 기능 추가 여부, 전략 결정 등 대표님만 결정 가능한 것 | 그때만 여쭤봄 |
...
| API 호출처 | 내용 |
|------|------|
| `api/cooldown.js` | 쿨다운 자동 게시 (claude-sonnet-4-6, 4회/일) |
...
### Privacy & Security
- **Data API**: ✅ 활성화 (2026-06-29 확인 — 이전 문서 오기재)
- **Workflow API**: 활성화
...
- **Cooldown Auto Post**: cron-job.org → Vercel /api/cooldown → Claude API → Bubble → 하루 4회 자동 게시 (08·12·18·21시 KST)
...
> ✅ **2026-06-29 업데이트**: 두 Search 모두에 `last_active > Current date/time + days: -90` 조건 추가.
> 90일 이상 미접속 유령 유저는 카드 후보에서 제외. 활성 풀 = 548명 (90일 기준).
> 이유: 유령 유저 카드 → "같이 달려요" 무응답 → 신뢰도 하락. 빈 카드보다 유령 카드가 더 나쁜 UX.
...
> 2026-06-22 업데이트: 페이서스 2.0 구독모델 확정 — 상세는 §31 참고

- **현재**: 인앱 결제 (티켓 시스템, 1티켓=450원)
- **확정 (페이서스 2.0)**: 티켓 모델 유지 + 구독 모델 신설 (공존), B2B 광고는 더 장기 과제
...
- **위치**: Daily Pacer 상세 카드
- **목적**: "같이 달려요" 전 워밍업 → 매칭 성사율 ↑ → 티켓 소모 ↑ → 수익 직결
- **리셋**: 데일리 카드 사이클(매일 정오 12시)과 동일하게 자동 리셋 — 별도 로직 불필요
...
```
내가 첫인상 선택 (⚡/🥇/🔥)
  → 카드 하단: "상대방에게 첫인상을 전달했어요" (버블 컨디셔널)
  → 상대방: 앱 내 알림 + OneSignal 푸시

상대방도 내 카드에서 첫인상 선택
  → 양쪽 카드에: "서로 첫인상을 보냈어요 👟 같이 달려볼까요?" 표시
```

**UI (Bubble에 구현됨 ✅):**
- "Q.[닉네임]의 첫인상은 어떤가요?" 질문
- ⚡ **페이스 딱 맞겠다**
- 🥇 **진짜 러너다**
- 🔥 **꾸준함이 보여**
...
| `created_date` | date | 생성 일시 (카드 사이클 체크용) |
| `is_mutual` | yes/no | 상호 첫인상 여부 |

**왜 User 필드 아닌 별도 타입?**
- 히스토리 이력 저장 가능 (보낸/받은 첫인상 전체 조회)
...
1. **신규 유료 기능 2개**: "오늘의 카드"(사주/타로, §32) + 마라톤 훈련 플랜(§30) — 둘 다 티켓 차감
2. **기존 유료 기능도 티켓 차감 유지**: 같이 달려요(1:1 호감), 데일리크루 호감 등
...
- 티켓을 없애지 않는 이유: 가볍게 쓰는 유저(저빈도)에게는 여전히 티켓이 합리적 — 구독 강제 시 이탈 위험. 공존 구조가 두 세그먼트 모두 커버
- 신규 유료 기능(오늘의 카드 + 마라톤 플랜)이 "구독 전환 트리거" 역할 — 기존 매칭 기능만으론 구독 명분이 약했는데, 콘텐츠 소비형 기능이 늘면서 "월정액으로 다 쓰는게 낫다"는 계산이 쉬워짐

### 미확정 / 다음 단계
- [ ] 정확한 구독 가격 확정 (5,500~7,800원 범위 내)
- [ ] 정확한 티켓 인상값 확정 (호감보내기 3→5 또는 3→6)
- [ ] RevenueCat에 구독(Subscription) Offering/Entitlement 신규 생성 — 기존 소비성 SKU와 별도 관리
- [ ] 구독자 판별 로직: Bubble User 테이블에 `is_subscriber`(yes/no) 또는 `subscription_entitlement` 필드 추가 → 모든 티켓 차감 워크플로우 앞단에 "구독자면 차감 스킵" 분기 추가 필요 (같이 달려요, 데일리크루, 오늘의 카드, 마라톤 플랜 등 전체 영향)
- [ ] RevenueCat webhook에 구독 갱신/해지 이벤트(`RENEWAL`, `CANCELLATION`, `EXPIRATION`) 핸들링 추가 — 기존 §27 webhook은 소비성 구매(`INITIAL_PURCHASE`/`NON_SUBSCRIPTION_PURCHASE`)만 처리하므로 확장 필요
- [ ] 티켓 가격 인상 공지 — 기존 유저 반발 최소화 방안 (예: 기존 보유 티켓은 영향 없음, 인상 전 사전 안내 등) 검토
...
| 시나리오 | 시간 | 내용 |
|---------|------|------|
| Make.com 6263718 | 22:00 KST | 스토리 아크 5장 (`?arc=1`, 풀 우선 → Sonnet 폴백) |
| Make.com 6270177 | 19:30 KST | 요일별 러닝 팁 5장 (풀 우선 → Sonnet 폴백, 실제 뉴스 아님) |
...
### 🔴🔴 최우선 블로커 (이거부터 풀어야 함 — 안 풀면 계속 도돌이표)
**증상 2개 (같은 뿌리):**
1. 진화 팝업 "확인" 버튼이 해상도에 따라 안 눌림
2. 냥이 상세 세로 스크롤이 특정 해상도에서 먹통

**근본 원인 = 한 iframe 안에서 "긴 스크롤 콘텐츠" + "전체화면 오버레이(position:fixed)"를 동시에 하려는 구조 충돌:**
- iframe=`100vh`+내부스크롤 → 오버레이는 화면 기준으로 잘 뜸. BUT 버블 박스가 화면보다 크면 페이지 스크롤과 충돌 → 특정 해상도 스크롤 먹통
- iframe=`2100px`+페이지스크롤 → 스크롤은 잘 됨. BUT 오버레이가 2100px 좌표 한가운데 떠서 화면 밖 / 확인버튼 클릭 안 됨
- **이 둘은 양립 불가. 한쪽 고치면 다른 쪽 깨짐. (이번 세션 내내 반복한 핵심 원인)**

**✅ 확정 해결책 (다음 세션 첫 작업): 오버레이를 iframe 밖으로 빼서 둘을 분리한다.**
구조:
1. **cat_detail iframe = 순수 스크롤 콘텐츠만** → `2100px`(또는 콘텐츠 높이) + 버블 박스 fit-content → 페이지 스크롤 (날씨 위젯 2200px처럼 검증된 방식, 어떤 해상도든 됨). iframe 안의 진화/졸업/토스트 오버레이 코드는 **제거 또는 비활성**.
2. **진화/졸업 연출 = 버블 네이티브 Popup** (cat_input 입력폼이랑 똑같은 패턴):
   - `didRun` 핸들러가 레벨업/졸업 감지 → `window.parent.postMessage({action:'show_evo', from, to, charName, line}, '*')`
   - 버블 다리 스크립트가 받아서 → 버블 "진화 Popup" Show
   - Popup 안엔 작은 iframe `cat_evo.html?from=&to=&name=` (진화 비주얼) 또는 버블 네이티브 요소
   - "확인" = **버블 네이티브 버튼** (Hide Popup) → 어떤 해상도든 무조건 닫힘 ✅
3. 밥먹기/하트/토스트 같은 작은 연출은 iframe 안에 둬도 됨(전체화면 아니라 상관없음) — 단 `position:fixed`로 화면 전체 덮는 것만 빼면 됨.

**왜 이게 정답인가:** 스크롤=페이지스크롤(견고), 오버레이=버블Popup(견고). 서로 안 건드림. position:fixed-in-tall-iframe 문제 자체가 사라짐. (입력 팝업을 이 방식으로 바꿔서 이미 잘 작동 중 = 검증된 패턴)

**작업 순서:**
1. cat_detail를 2100px 페이지스크롤로 고정 (박스 3개 fit-content) → 스크롤부터 확실히
2. `cat_evo.html` 신규 생성 (진화 비주얼 iframe, 작게)
3. cat_detail didRun → 레벨업 시 show_evo postMessage (in-iframe 오버레이 호출 제거)
4. 버블: 진화 Popup + 다리에 show_evo 수신 + 작은 cat_evo iframe
5. 졸업도 동일 패턴

> 대안(비추): 100vh 유지 + My Cat 탭/페이지를 viewport높이로 고정해 페이지스크롤 자체 제거. 단 버블에서 박스=viewport 맞추기가 까다로워 또 도돌이표 위험. **위 "오버레이 분리"가 정공법.**
...
### API 접근 정보
- **앱 URL**: `https://pacers.kr`
- **Data API base**: `https://pacers.kr/version-test/api/1.1/obj/`
- **Workflow API base**: `https://pacers.kr/version-test/api/1.1/wf/`
- **Admin API Token**: 환경변수 `BUBBLE_API_TOKEN` 또는 로컬에서 직접 확인 (Bubble Settings → API)
...
| `photo_1~4` | 프로필 사진 |
| `gender` | 성별 |
| `birth_date`, `birth_year` | 생년월일 |
| `height_cm` | 키 |
| `mbti` | MBTI |
| `job` | 직업 |
| `paceRange` | 페이스 범위 |
| `race_experience` | 대회 경험 |
| `home_address`, `home_address_kor` | 주소 |
| `last_location` | 최근 위치 |
| `tickets` | 티켓 수 |
| `today_cards` | 오늘 카드 목록 |
| `today_match_done` | 오늘 매칭 완료 여부 |
| `last_active`, `last_login` | 최근 활동/로그인 |
| `is_onboarded`, `onboarding_skipped` | 온보딩 상태 |
| `is_writing_cooldown`, `is_writing_dailycrew` | 작성 중 상태 |
| `age_score`, `distance_score`, `gender_score`, `pace_score`, `pace_match_score` | 매칭 점수 |
| `viewed_ids`, `viewed_today_list`, `viewed_users_total` | 조회 기록 |
| `new_running_meetstyle`, `new_runningpurpose`, `userTrait` | 러닝 스타일 |
| `preferred_gender` | 선호 성별 |
| `extra_card_count`, `extra_card_reset_time` | 추가 카드 |
| `popup_snooze_until` | 팝업 스누즈 |
| `dailycrew_likes_sent` | 데일리크루 좋아요 |
| `invite_code`, `is_invited` | 초대 코드 |
| `badges` | 뱃지 |
| `phone_number` | 전화번호 |
...
| `emotion` | 감정/컨디션 |
| `comment` | 메모 |
| `is_rewarded` | 보상 지급 여부 |
| `month_key`, `week_key`, `year_key` | 기간 키 |
...
| `cd_title` | 제목 |
| `cd_category` | 카테고리 |
| `cd_post_author` | 작성자 (User ref) |
| `cd_is_anonymous` | 익명 여부 |
| `cd_an_nickname` | 익명 닉네임 |
| `cd_post_likes`, `cd_post_likes_count` | 좋아요 |
| `cd_comments`, `cd_commenters` | 댓글 |
| `cd_views` | 조회수 |
| `cd_is_visible`, `is_deleted`, `is_hidden` | 노출 상태 |
| `cd_created_date` | 작성일 |

### 마라톤 대회 필드 (API명: `%EB%A7%88%EB%9D%BC%ED%86%A4%20%EB%8C%80%ED%9A%8C`)
| 필드 | 설명 |
|------|------|
| `마라톤 대회명` | 대회명 |
| `대회 날짜`, `대회 날짜 시스템` | 날짜 |
| `거리 종류` | 5K/10K/하프/풀 |
| `지역`, `지역 그룹` | 지역 |
| `집결 장소`, `집결 시간`, `Meeting_Time` | 집결 정보 |
| `접수 상태`, `접수 시작일_date`, `접수 마감일`, `접수 마감일_date` | 접수 정보 |
| `마라톤 대회 설명`, `대회 상세 타이틀`, `대회 상세 콘텐츠` | 상세 내용 |
| `마라톤 메인 이미지` | 이미지 |
| `공식 홈페이지` | 홈페이지 URL |
| `주최 기관` | 주최사 |
| `해쉬 태그` | 해시태그 |
...
### OpenRun 타입
- **OpenRun_Course**: 코스 정보 (Main_Title, Course_Distance, Course_Region, Meeting_Point 등)
- **OpenRun_Group**: 그룹 (Members, Match_Date, Linked_Course 등)
- **OpenRun_Entry**: 참가 신청 (Applicant_User, Match_Status, Is_Leader 없음)

---

## § 34: 2026-06-29 작업 이력 & 라이브 지표

### ✅ 완료

**Bubble Data API 라이브 활성화 확인**
- 엔드포인트: `https://pacers.kr/api/1.1/obj/[type]` (live)
- `version-test`는 별도 DB — 반드시 live 엔드포인트 사용
- 인증: `Authorization: Bearer 36aa51d61c70995e64e40de1f78a3fa6`

**api/monitor.js 전면 개선 (Vercel 배포 완료)**
- 기상청 날씨 API 헬스체크 추가
- 에어코리아 대기질 API 헬스체크 추가
- cron-job.org 쿨다운 잡 4개 마지막 실행 상태 추가 (CRONJOB_API_KEY 환경변수)
- Slack 리포트 구조화 (날씨/쿨다운/시스템 섹션 분리)

**generate_daily_cards_v5 유령 유저 필터 추가 (버블 직접 수정)**
- Search for Users (10km) + 1000km fallback 두 곳 모두에 `last_active > -90일` 조건 추가
- 활성 풀: 전체 1,125명 → 548명 (90일 이내 접속자만)
...
| 2026년 2월 | 58명 | 2명 | 3% |
| 2026년 3월 | 34명 | 3명 | 9% |
| 2026년 4월 | 102명 | 16명 | 16% |
| 2026년 5월 | 291명 | 109명 | 37% |
| 2026년 6월 | 226명 | 174명 | 77%* |

*6월은 신규라 의미 없는 수치. 5월 37%가 실질 리텐션 지표.

**업계 벤치마크 (출처: Adjust 2026, AppsFlyer Q3 2024)**
- 데이팅 앱 D30 글로벌 평균: 5~6%
- 전체 모바일 앱 평균: 7%
- 골드스푼 1달 리텐션: 10~20% (CEO 발언)
- **페이서스 5월 코호트: 37%** → 업계 평균 6배, 골드스푼보다 높음
- 결론: 지표 자체는 좋음. 문제는 모수(신규 유입)가 작은 것

**인스타 마케팅 현황**
- 팔로워: 약 400명
- 릴스 평균 조회수: ~1,000회 (팔로워 대비 2.5배, 외부 확산 약함)
- 카드뉴스 자동 발행: 1일 2개 (22:00 스토리아크, 19:30 팁, 22:30 월수금 클투스타일)
- 인스타 광고: 2~3만원/일 → 월 70만원 → 5월 291명 유입 (CAC ≈ 2,400원/명)

### 🔴 다음 세션 우선순위 (7월 목표)
1. **다마고치** — DAU 개선 핵심. 매일 앱 열 이유. `cat.html` + `cat_detail.html` 구현
2. **마라톤 훈련 플랜** — 버블 연동 완성 (§30 참고)
